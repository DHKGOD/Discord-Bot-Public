...
class Snipe(ExtensionBase):
    ...
    async def on_message_delete(self, message):
        ...
        SuperEmbed.add_field(name='頻道名子', value=">  " + "#" + message.channel.name + " ", inline=True)
    ...
    @commands.command(aliases=['Sinpe'])
    async def snipe(self, ctx):
        ...
        try:
            ...
            snipeEmbed = discord.Embed(title="> " + str(snipe_message_content[channel.id]), color=0xDAA520)
            ...
            t = passtime
            h, m, s = t.split(':')
            snipeEmbed.add_field(name='** **', value="🗑 "+str(datetime.timedelta(hours=int(h), minutes=int(m), seconds=int(s)).total_seconds()) + "s ago", inline=True)
